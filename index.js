...
class ResultsScreen extends GameObject {
    ...
    this.on(`mount`, () => this.submitScore())
	}
	updateTrophy() {
	...
	async entryAnimation() {
		...
		await Animate.blink(title, { duration: 600 }).promise
		await Animate.slide(title, { duration: 300, delay: 200, to: ogPos }).promise
		...
		await Promise.all([
			Animate.fadeIn(scoreLabel, { duration: 300 }).promise,
			Animate.fadeIn(score, { duration: 300 }).promise,
		])
...
(async function () {
	...
	// 	: new OpeningScreen()
	// )
  Game.root.getChild(`main`).addChild(new Menu({ animate: false }))
